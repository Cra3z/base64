...
template<typename T>
struct is_range {
    static constexpr bool value = std::ranges::range<T>;
};
...
auto encode_to(ForwardIt first, ForwardIt second, OutIt target) -> void {
    static_assert(std::is_base_of<std::forward_iterator_tag, typename ForwardIt::iterator_category>::value, "type `ForwardIt` must be a forward-iterator type");
    static_assert(
            std::is_base_of<std::forward_iterator_tag, typename OutIt::iterator_category>::value ||
                    std::is_base_of<std::output_iterator_tag, typename OutIt::iterator_category>::value,
            "type `OutIt` must be a forward-iterator or output-iterator type");
    static_assert(detail::is_byte_type<typename ForwardIt::value_type>::value, "the type of value in the range [first, second) must be `char`, `unsigned char`, `signed char`, `byte` or `std::byte`");
